...
AlVideoCapturerThread::AlVideoCapturerThread(cricket::VideoCapturer *pCapturer)
{
        ...
        while (!stopped) {
            ...
            if (m_pImageBytes && m_imageDataLen > 0 && m_width > 0 && m_height > 0)
            {
                ...
                if (frame.width*frame.height*4 != m_imageDataLen)
                    std::cout << "AlVideoCapturerThread: with and height don't match size for ABGR data, skipping frame" << std::endl;
                else
                {
