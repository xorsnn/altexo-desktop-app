...
extern "C" void destroy_object(AlManagerInterface *object) { delete object; }

AlManagerInterface::AlManagerInterface() { m_manager = new AlManager(); }
...
void AlManagerInterface::run() { m_manager->run(); }
...
void AlManagerInterface::OnMessageFromPeer(std::string peerId,
...
void AlManagerInterface::clientSignOut() { m_manager->m_client.SignOut(); }

void AlManagerInterface::setDesiredDataSource(int dataSource) {
  m_manager->setDesiredDataSource(dataSource);
}

void AlManagerInterface::setImageData(uint8_t *pImageBytes, size_t len,
