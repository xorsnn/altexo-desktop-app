...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
  // Setting those fields so that SetScrollHere() can properly function after
...
bool ImGuiListClipper::Step() {
                   ...
                   // position the cursor before the first element.
  {
    ...
    ImGui::SetCursorPosY(StartPosY); // Rewind cursor so we can Begin() again,
                                     // this time with a known height.
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size,
  ...
  if (g.ActiveId == id || (edit_state.Id == id && is_multiline &&
                           g.ActiveId == draw_window->GetID("#SCROLLY"))) {
